...
// Patch all .d.ts files: replace '@lidofinance/lido-shared' with correct relative path
function patchFile(filePath) {
  const content = fs.readFileSync(filePath, 'utf8')
  if (!content.includes('@lidofinance/lido-shared')) return
...
walk(distTypes)
console.log('Patched @lidofinance/lido-shared references in dist/types/')
