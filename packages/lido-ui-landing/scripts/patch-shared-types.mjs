...
// Patch all .d.ts files: replace '@lido-ui/shared' with correct relative path
function patchFile(filePath) {
  const content = fs.readFileSync(filePath, 'utf8')
  if (!content.includes('@lido-ui/shared')) return
  ...
  const patched = content.replaceAll("'@lido-ui/shared'", `'${relSpec}'`)
...
walk(distTypes)
console.log('Patched @lido-ui/shared references in dist/types/')
