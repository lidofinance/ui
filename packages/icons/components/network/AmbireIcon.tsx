// THIS FILE IS AUTO GENERATED

import React, { SVGProps } from 'react'

export const AmbireIcon = (props: SVGProps<SVGSVGElement>) => {
  return (
    <svg width={81} height={80} viewBox='0 0 81 80' fill='none' {...props}>
      <g filter='url(#AmbireIcon__a)' opacity={0.5}>
        <path
          fill='#6000FF'
          fillRule='evenodd'
          d='M54.477 32.537l7.49 15.943a.452.452 0 01-.093.497l-19.7 19.035c-.148.129-.37.037-.37-.166V50.357L54.22 38.373c.092-.074.129-.203.129-.313l.037-5.523c0-.037.074-.037.092 0z'
          clipRule='evenodd'
        />
        <path
          fill='url(#AmbireIcon__b)'
          fillRule='evenodd'
          d='M62.004 48.756a.481.481 0 00-.037-.276l-7.49-15.943c-.018-.036-.092-.036-.092.019l-.037 5.523a.353.353 0 01-.074.22l7.73 10.457z'
          clipRule='evenodd'
        />
        <path
          fill='#fff'
          fillRule='evenodd'
          d='M62.004 48.756a.482.482 0 00-.037-.276l-.388-.829-6.825-8.744 7.25 9.849z'
          clipRule='evenodd'
          opacity={0.2}
        />
        <g opacity={0.5}>
          <mask
            id='AmbireIcon__a'
            width={22}
            height={37}
            x={41}
            y={32}
            maskUnits='userSpaceOnUse'
          >
            <path
              fill='#fff'
              d='M54.477 32.537l7.49 15.943a.452.452 0 01-.093.497l-19.7 19.035c-.148.129-.37.037-.37-.166V50.357L54.22 38.373c.092-.074.129-.203.129-.313l.037-5.523c0-.037.074-.037.092 0z'
              opacity={0.5}
            />
          </mask>
          <g mask='url(#AmbireIcon__a)'>
            <path
              fill='url(#AmbireIcon__c)'
              d='M57.577 32.353H30.829v42.342h26.748V32.353z'
            />
          </g>
        </g>
        <path
          fill='#6000FF'
          fillRule='evenodd'
          d='M36.769 19.153l-3.524 9.978a.46.46 0 00.019.35l3.283 6.462-9.039 5.118c-.11.055-.258.018-.313-.093L25.239 36.9a.392.392 0 01.037-.423l11.4-17.379c.037-.037.111 0 .093.055z'
          clipRule='evenodd'
        />
        <path
          fill='url(#AmbireIcon__AmbireIcon__e)'
          fillRule='evenodd'
          d='M33.208 29.26c0-.037 0-.074.019-.11l3.523-9.997c.019-.055-.055-.073-.074-.036l-10.939 16.66 5.24-4.474-5.756 5.284 7.987-7.327z'
          clipRule='evenodd'
        />
        <path
          fill='url(#AmbireIcon__AmbireIcon__f)'
          fillRule='evenodd'
          d='M25.737 35.759l-.46.7c-.02.036-.038.073-.056.11l5.755-5.284-5.239 4.474z'
          clipRule='evenodd'
        />
        <path
          fill='#fff'
          fillRule='evenodd'
          d='M25.737 35.759l-.46.7c-.02.036-.038.073-.056.11l5.755-5.284-5.239 4.474z'
          clipRule='evenodd'
          opacity={0.2}
        />
        <g opacity={0.4}>
          <path
            fill='#6000FF'
            fillRule='evenodd'
            d='M36.769 19.153l-3.524 9.978a.46.46 0 00.019.35l3.283 6.462-9.039 5.118c-.11.055-.258.018-.313-.093L25.239 36.9a.392.392 0 01.037-.423l11.4-17.379c.037-.037.111 0 .093.055z'
            clipRule='evenodd'
            opacity={0.4}
          />
          <g opacity={0.4}>
            <mask
              id='AmbireIcon__b'
              width={12}
              height={23}
              x={25}
              y={19}
              maskUnits='userSpaceOnUse'
            >
              <path
                fill='#fff'
                d='M36.769 19.153l-3.524 9.978a.46.46 0 00.019.35l3.283 6.462-9.039 5.118c-.11.055-.258.018-.313-.093L25.239 36.9a.392.392 0 01.037-.423l11.4-17.379c.037-.037.111 0 .093.055z'
                opacity={0.4}
              />
            </mask>
            <g mask='url(#AmbireIcon__b)'>
              <path
                fill='url(#AmbireIcon__AmbireIcon__h)'
                d='M49.091 28.119H16.256v24.668H49.09V28.12z'
              />
            </g>
          </g>
        </g>
        <path
          fill='url(#AmbireIcon__AmbireIcon__i)'
          fillRule='evenodd'
          d='M41.97 11.053l8.32 20.361a.51.51 0 010 .331L36.86 63.17a.212.212 0 01-.35.074l-12.36-11.93a.426.426 0 01-.055-.57l17.617-23.877a.439.439 0 00.092-.258V11.09c0-.092.13-.129.167-.037z'
          clipRule='evenodd'
        />
        <path
          fill='url(#AmbireIcon__AmbireIcon__j)'
          fillRule='evenodd'
          d='M43.39 27.659l-1.31-.737 8.099 4.235-8.209-20.104c-.037-.092-.166-.055-.166.037v15.52c0 .036 0 .091-.018.128l1.605.92z'
          clipRule='evenodd'
        />
        <path
          fill='url(#AmbireIcon__AmbireIcon__k)'
          fillRule='evenodd'
          d='M43.39 27.659l6.918 3.92c0-.055-.018-.091-.037-.146l-.11-.277-8.099-4.234 1.329.736z'
          clipRule='evenodd'
        />
        <path
          fill='#fff'
          fillRule='evenodd'
          d='M43.39 27.659l6.918 3.92c0-.055-.018-.091-.037-.146l-.11-.277-8.099-4.234 1.329.736z'
          clipRule='evenodd'
          opacity={0.2}
        />
      </g>
      <path
        fill='#6000FF'
        fillRule='evenodd'
        d='M50.86 33.377l7.086 15.084a.427.427 0 01-.088.47l-18.64 18.01c-.14.122-.349.035-.349-.156V50.238l11.746-11.34c.088-.07.122-.191.122-.296l.035-5.225c0-.035.07-.035.088 0z'
        clipRule='evenodd'
      />
      <path
        fill='url(#AmbireIcon__AmbireIcon__l)'
        fillRule='evenodd'
        d='M57.98 48.723a.455.455 0 00-.035-.262L50.86 33.377c-.017-.034-.087-.034-.087.018l-.035 5.225c0 .07-.017.14-.07.21l7.313 9.893z'
        clipRule='evenodd'
      />
      <path
        fill='#fff'
        fillRule='evenodd'
        d='M57.98 48.722a.457.457 0 00-.034-.26l-.367-.785-6.458-8.273 6.86 9.318z'
        clipRule='evenodd'
        opacity={0.2}
      />
      <g opacity={0.5}>
        <mask
          id='AmbireIcon__d'
          width={20}
          height={34}
          x={38}
          y={33}
          maskUnits='userSpaceOnUse'
        >
          <path
            fill='#fff'
            d='M50.86 33.377l7.086 15.084a.427.427 0 01-.088.47l-18.64 18.01c-.14.122-.349.035-.349-.156V50.238l11.746-11.34c.088-.07.122-.191.122-.296l.035-5.225c0-.035.07-.035.088 0z'
            opacity={0.5}
          />
        </mask>
        <g mask='url(#AmbireIcon__d)'>
          <path
            fill='url(#AmbireIcon__AmbireIcon__n)'
            d='M53.792 33.203H28.484v40.061h25.308V33.203z'
          />
        </g>
      </g>
      <path
        fill='#6000FF'
        fillRule='evenodd'
        d='M34.104 20.714l-3.333 9.44a.436.436 0 00.017.332l3.107 6.113-8.552 4.843c-.105.052-.245.017-.297-.088l-1.85-3.849a.37.37 0 01.035-.4l10.786-16.443c.035-.035.105 0 .087.052z'
        clipRule='evenodd'
      />
      <path
        fill='url(#AmbireIcon__AmbireIcon__o)'
        fillRule='evenodd'
        d='M30.736 30.277c0-.035 0-.07.017-.105l3.334-9.458c.017-.052-.052-.07-.07-.035l-10.35 15.764 4.957-4.233-5.445 4.999 7.557-6.932z'
        clipRule='evenodd'
      />
      <path
        fill='url(#AmbireIcon__AmbireIcon__p)'
        fillRule='evenodd'
        d='M23.667 36.425l-.436.662-.052.105 5.445-5-4.957 4.233z'
        clipRule='evenodd'
      />
      <path
        fill='#fff'
        fillRule='evenodd'
        d='M23.667 36.425l-.436.662-.052.105 5.445-5-4.957 4.233z'
        clipRule='evenodd'
        opacity={0.2}
      />
      <g opacity={0.4}>
        <path
          fill='#6000FF'
          fillRule='evenodd'
          d='M34.104 20.714l-3.333 9.44a.436.436 0 00.017.332l3.107 6.113-8.552 4.843c-.105.052-.245.017-.297-.088l-1.85-3.849a.37.37 0 01.035-.4l10.786-16.443c.035-.035.105 0 .087.052z'
          clipRule='evenodd'
          opacity={0.4}
        />
        <g opacity={0.4}>
          <mask
            id='AmbireIcon__c'
            width={12}
            height={22}
            x={23}
            y={20}
            maskUnits='userSpaceOnUse'
          >
            <path
              fill='#fff'
              d='M34.104 20.714l-3.333 9.44a.436.436 0 00.017.332l3.107 6.113-8.552 4.843c-.105.052-.245.017-.297-.088l-1.85-3.849a.37.37 0 01.035-.4l10.786-16.443c.035-.035.105 0 .087.052z'
              opacity={0.4}
            />
          </mask>
          <g mask='url(#AmbireIcon__c)'>
            <path
              fill='url(#AmbireIcon__AmbireIcon__r)'
              d='M45.763 29.197H14.696v23.34h31.067v-23.34z'
            />
          </g>
        </g>
      </g>
      <path
        fill='url(#AmbireIcon__AmbireIcon__s)'
        fillRule='evenodd'
        d='M39.026 13.05l7.872 19.265a.483.483 0 010 .313L34.192 62.361a.2.2 0 01-.332.07L22.166 51.142a.403.403 0 01-.052-.54l16.668-22.59a.415.415 0 00.087-.244V13.085c0-.087.122-.122.157-.035z'
        clipRule='evenodd'
      />
      <path
        fill='url(#AmbireIcon__AmbireIcon__t)'
        fillRule='evenodd'
        d='M40.37 28.762l-1.24-.697 7.663 4.006-7.767-19.02c-.035-.088-.157-.053-.157.034V27.77c0 .035 0 .087-.017.122l1.518.87z'
        clipRule='evenodd'
      />
      <path
        fill='url(#AmbireIcon__AmbireIcon__u)'
        fillRule='evenodd'
        d='M40.37 28.761l6.545 3.71c0-.052-.017-.087-.035-.139l-.105-.261-7.662-4.006 1.257.696z'
        clipRule='evenodd'
      />
      <path
        fill='#fff'
        fillRule='evenodd'
        d='M40.37 28.761l6.545 3.71c0-.052-.017-.087-.035-.139l-.105-.261-7.662-4.006 1.257.696z'
        clipRule='evenodd'
        opacity={0.2}
      />
      <defs>
        <linearGradient
          id='AmbireIcon__AmbireIcon__b'
          x1={54.75}
          x2={62.614}
          y1={31.092}
          y2={37.579}
          gradientUnits='userSpaceOnUse'
        >
          <stop stopColor='#AF50FF' stopOpacity={0} />
          <stop offset={1} stopColor='#AF50FF' />
        </linearGradient>
        <linearGradient
          id='AmbireIcon__AmbireIcon__e'
          x1={36.516}
          x2={22.294}
          y1={21.828}
          y2={31.117}
          gradientUnits='userSpaceOnUse'
        >
          <stop stopColor='#AF50FF' stopOpacity={0} />
          <stop offset={1} stopColor='#AF50FF' />
        </linearGradient>
        <linearGradient
          id='AmbireIcon__AmbireIcon__f'
          x1={36.263}
          x2={27.248}
          y1={21.661}
          y2={37.733}
          gradientUnits='userSpaceOnUse'
        >
          <stop stopColor='#AF50FF' stopOpacity={0} />
          <stop offset={1} stopColor='#AF50FF' />
        </linearGradient>
        <linearGradient
          id='AmbireIcon__AmbireIcon__i'
          x1={4.959}
          x2={52.061}
          y1={87.411}
          y2={69.976}
          gradientUnits='userSpaceOnUse'
        >
          <stop stopColor='#AF50FF' />
          <stop offset={1} stopColor='#6000FF' />
        </linearGradient>
        <linearGradient
          id='AmbireIcon__AmbireIcon__j'
          x1={41.571}
          x2={53.851}
          y1={18.533}
          y2={21.068}
          gradientUnits='userSpaceOnUse'
        >
          <stop stopColor='#AF50FF' stopOpacity={0} />
          <stop offset={1} stopColor='#AF50FF' />
        </linearGradient>
        <linearGradient
          id='AmbireIcon__AmbireIcon__k'
          x1={39.069}
          x2={42.818}
          y1={20.626}
          y2={34.611}
          gradientUnits='userSpaceOnUse'
        >
          <stop stopColor='#AF50FF' stopOpacity={0} />
          <stop offset={1} stopColor='#AF50FF' />
        </linearGradient>
        <linearGradient
          id='AmbireIcon__AmbireIcon__l'
          x1={51.118}
          x2={58.558}
          y1={32.01}
          y2={38.148}
          gradientUnits='userSpaceOnUse'
        >
          <stop stopColor='#AF50FF' stopOpacity={0} />
          <stop offset={1} stopColor='#AF50FF' />
        </linearGradient>
        <linearGradient
          id='AmbireIcon__AmbireIcon__o'
          x1={33.866}
          x2={20.41}
          y1={23.245}
          y2={32.034}
          gradientUnits='userSpaceOnUse'
        >
          <stop stopColor='#AF50FF' stopOpacity={0} />
          <stop offset={1} stopColor='#AF50FF' />
        </linearGradient>
        <linearGradient
          id='AmbireIcon__AmbireIcon__p'
          x1={33.627}
          x2={25.096}
          y1={23.087}
          y2={38.293}
          gradientUnits='userSpaceOnUse'
        >
          <stop stopColor='#AF50FF' stopOpacity={0} />
          <stop offset={1} stopColor='#AF50FF' />
        </linearGradient>
        <linearGradient
          id='AmbireIcon__AmbireIcon__s'
          x1={4.008}
          x2={48.573}
          y1={85.296}
          y2={68.8}
          gradientUnits='userSpaceOnUse'
        >
          <stop stopColor='#AF50FF' />
          <stop offset={1} stopColor='#6000FF' />
        </linearGradient>
        <linearGradient
          id='AmbireIcon__AmbireIcon__t'
          x1={38.649}
          x2={50.267}
          y1={20.127}
          y2={22.525}
          gradientUnits='userSpaceOnUse'
        >
          <stop stopColor='#AF50FF' stopOpacity={0} />
          <stop offset={1} stopColor='#AF50FF' />
        </linearGradient>
        <linearGradient
          id='AmbireIcon__AmbireIcon__u'
          x1={36.281}
          x2={39.828}
          y1={22.108}
          y2={35.339}
          gradientUnits='userSpaceOnUse'
        >
          <stop stopColor='#AF50FF' stopOpacity={0} />
          <stop offset={1} stopColor='#AF50FF' />
        </linearGradient>
        <pattern
          id='AmbireIcon__AmbireIcon__d'
          width={1}
          height={1}
          patternContentUnits='objectBoundingBox'
        >
          <use
            href='#AmbireIcon__AmbireIcon__v'
            transform='scale(.0069 .00435)'
          />
        </pattern>
        <pattern
          id='AmbireIcon__AmbireIcon__h'
          width={1}
          height={1}
          patternContentUnits='objectBoundingBox'
        >
          <use
            href='#AmbireIcon__AmbireIcon__w'
            transform='scale(.00562 .00746)'
          />
        </pattern>
        <pattern
          id='AmbireIcon__AmbireIcon__n'
          width={1}
          height={1}
          patternContentUnits='objectBoundingBox'
        >
          <use
            href='#AmbireIcon__AmbireIcon__v'
            transform='scale(.0069 .00435)'
          />
        </pattern>
        <pattern
          id='AmbireIcon__AmbireIcon__r'
          width={1}
          height={1}
          patternContentUnits='objectBoundingBox'
        >
          <use
            href='#AmbireIcon__AmbireIcon__w'
            transform='scale(.00562 .00746)'
          />
        </pattern>
        <image
          id='AmbireIcon__AmbireIcon__v'
          width={145}
          height={230}
          href='data:image/png;base64,'
          preserveAspectRatio='none'
        />
        <image
          id='AmbireIcon__AmbireIcon__w'
          width={178}
          height={134}
          href='data:image/png;base64,'
          preserveAspectRatio='none'
        />
        <filter
          id='AmbireIcon__AmbireIcon__a'
          width={60}
          height={79.072}
          x={13.011}
          y={0}
          colorInterpolationFilters='sRGB'
          filterUnits='userSpaceOnUse'
        >
          <feFlood floodOpacity={0} result='BackgroundImageFix' />
          <feBlend in='SourceGraphic' in2='BackgroundImageFix' result='shape' />
          <feGaussianBlur
            result='effect1_foregroundBlur_52_17179'
            stdDeviation={5.5}
          />
        </filter>
      </defs>
    </svg>
  )
}
