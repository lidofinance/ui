...
У нас CSS-модули и непубликуемый `@lido-ui/shared` — tsdown не умеет нормально работать с обоими.
...
### 2. `@lido-ui/shared` не публикуется
...
В tsdown для этого нужен `noExternal: ['@lido-ui/shared']`, но это работает только для JS-части. CSS-файлы из shared при этом не обрабатываются postcss и теряют скопированные классы.
